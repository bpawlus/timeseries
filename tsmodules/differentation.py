...
class DifferentiationModule(basemod.TSModule):
    ...
    def displayModule(self, ax, plotdf: DataFrame):
        ...
        df = plotdf.rename(columns={f'{plotdf.columns[1]}': f'{plotdf.columns[1]} {loader.lang["modules"]["differentiation"]["diffdisplay"]} {loader.lang["modules"]["differentiation"]["lag"]} {self.lagentryvar.get()}'})
        df = df[[col for col in df.columns]].groupby(df.columns[0]).sum()
        df = df.diff(periods=self.lagentryvar.get())
